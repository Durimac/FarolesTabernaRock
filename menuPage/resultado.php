...
<html>
    ...
    <body>
        ...
        <?php
            ...
            $search = normalizeData($search);
            $product_name_only = normalizeData($product_name_only);
            $product_name = normalizeData($product_name);
            $description_only = normalizeData($description_only);
            $description = normalizeData($description);
            $price_only = normalizeData($price_only);
            $price = normalizeData($price);
            $kind_only = normalizeData($kind_only);
            $kind = normalizeData($kind);
            $calories_only = normalizeData($calories_only);
            $calories = normalizeData($calories);

            if	((!$search) && (!$product_name_only) && (!$product_name) && (!$description_only ) &&
                (!$description) && (!$price_only) && (!$price) && (!$kind_only) && (!$kind) &&
                (!$calories_only) && (!$calories)) {
                echo "<script type=\"text/javascript\">alert('Introduzca al menos un dato.')</script>";
                exit; 
            }
            /*
            $search=trim($search);
