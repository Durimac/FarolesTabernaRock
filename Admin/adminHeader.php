<!DOCTYPE html>
<html>

<head>
	<meta charset="UTF-8" />
	<meta name="title" content="Faroles Taberna Rock" />
	<meta name="copyright" content="(c) 2018 Universidad de Valladolid" />
	<meta name="author" content="Adrian Mazaira, Javier Olmedo y Darío Yuste" />
	<meta name="contact" content="bbb@hotmail.com" />
	<meta name="organization" content="Faroles Tabera Rock" />
	<meta name="homepage" content="index.html" />
	<meta name="revision" content="18 Oct 2018" />
	<meta name="keywords" lang="es" content="Faroles, Taberna, Valladolid, Tortilla, Rock, Heavy, Vegano, Perros, Bar, Cerveza, Casera, Coroqueta, MAES" />
	<meta name="description" content="Taberna Rockera donde prima la diversion y el buen ambiente, admitimos perros, vegetarianos y veganos" />


	<title>FarolesTabernaRock_Admin_MainPage</title>
	<link rel="stylesheet" href="../adminHeaderStyle.css" type="text/css" />
	<link rel="stylesheet" href="./style.css" type="text/css" />
</head>

<body>
	<header>
		<div class="header">
			<a class="logo" href="../ordersList/ordersList.php">
				<img class="logo" src="../../photos/adminLogo.png" alt="Logo Faroles Taberna Rock"/>
			</a>

			<!-- Row Menu -->
			<nav class="headerMenu">
				<a class="menu" href="../products/products.php">Productos</a>
				<a class="menu" href="../penistas/penistas.php">Peñistas</a>
				<a class="menu" href="../events/events.php">Eventos</a>
				<div style="height: 40%;">
					<img class="adminLogo" src="../../photos/exit.png" alt="Salir" onclick="confirmLogOut()"/>
				</div>
			</nav>
		</div>
	</header>
	
	<script src="../adminFunctions.js" type="text/javascript"></script>
	
	